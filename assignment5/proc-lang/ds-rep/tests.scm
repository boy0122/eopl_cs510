...
;; PROC language - Dave tests based on Marc's LET tests and EOPL3 for PROC
...
(define proc-tests         ;; adapted from eopl tests
  (test-suite
   "tests for PROC language"
    ...
    ;test traceproc
    (check-equal? (run "let f = traceproc (x) -(x, 11)
                              in (f (f 77))")
                  55)
...
(define let-tests
  (test-suite
   ...
   ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
   ;test cond
   (check-equal? (run "cond zero?(0) ==> 2 end") 2)
   (check-equal? (run "cond zero?(1)  ==> 1 zero?(0) ==>3 end") 3)
