...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; extend the checker to handle ECPLICIT-REFS.
;; step 1:
;; 
 ...
 ;;;;;;;; tests ;;;;;;;;;;
(define tests-for-ref
  (test-suite
   "tests for CHECKED language, type check"
      ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;new added;;;;;;;;;;;;;;;;;;;;;;;;;;
 ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
 (display "expect: (refto int): \n")
 (display (tcheck "let x = newref(newref(0)) in deref(x)"))
 (display "\n")
 (display "expect: int \n")
 (display (tcheck "let x = newref(0) in let y = setref(x, 10) in deref(x)"))
 (display "\n")
 (display "expect: int \n")
 (display (tcheck "let x = newref(22) in let f = proc (z : int) let zz = newref(-(z,deref(x))) in deref(zz)
           in -((f 66), (f 55))"))
 (display "\n")
 (display "expect: int \n")
 (display (tcheck "let x = newref(2) in begin setref(x,3);deref(x) end"))
 (display "\n")
 (display "expect: Types didn't match \n")
 (display (tcheck "let x = newref(zero?(2)) in begin setref(x,3);deref(x) end"))
...
in letrec 
 ...
 ;; newly added for EXPLICIT-REFS
  (check-equal? (tcheck "newref(17)") '(refto int) "newref-test-1")
  (check-equal? (tcheck "setref(newref(17), 10)") 'void "setref-test-1")
  (check-equal? (tcheck "let x = newref(17) in x" ) '(refto int) "let-test-1")
  (check-equal? (tcheck "let x = newref(17) in deref(x)") 'int "let-test-2")
  
  (check-equal? (tcheck "-(deref(newref(10)), 2)") 'int "dif-test-1")
  ; this value-of test will produce error!
  ;(check-equal? (run "let x = newref(17) in deref(x)") 17 "let-test-2")
...
(run-tests tests-for-ref 'verbose)
