#lang eopl
 ...
 ;; plus tests-for-pair (near end of this file)
...
;; tests for pair types

 (define tests-for-pair
   
 (test-suite "tests for CHECKED language, for pair type"
         
 ; interpreter: make pairs and undo them
 (check-equal? (run "unpair x y = newpair(-(10,4),zero?(0)) in x") 6 "basic int")
 (check-equal? (run "unpair x y = newpair(-(10,4),zero?(2)) in y") #f "basic bool")
         
 (check-equal? (run "let m = newpair(-5,6) in unpair x y = m in -(x,y)") -11 "more complicated int")
 (check-equal? (run "let m = newpair(-5,zero?(6)) in unpair x y = m in if y then x else 10") 10 "more complicated bool")
         
 ; interpreter: simply make pairs (the tests involve expval->scheme-val for pairs)
 (check-equal? (run "newpair(0,0)") '(0 0) "simple pair")
 (check-equal? (run "let w = 0 in newpair(w, newpair(w,3))") '(0 (0 3)) "nested pair")
 
 ; type checker: make and undo pairs
 (check-equal? (tcheck "unpair x y = newpair(-(10,4),zero?(0)) in x") 'int "basic int type")
 (check-equal? (tcheck "unpair x y = newpair(-(10,4),zero?(2)) in y") 'bool "basic bool type")
 
 ; type checker: make pairs (the tests involve type-to-external-form)
 (check-equal? (tcheck "newpair(0,0)") '(pairof int int) "type of simple pair")
 (check-equal? (tcheck "let w = 0 in newpair(w,newpair(w,w))") '(pairof int (pairof int int)) "pair type nested")
         
 (check-equal? (tcheck "let m = newpair(-5,6) in unpair x y = m in -(x,y)") 'int "more complicated int type")
 
 ; type checker: procedures that input or output pairs
 (check-equal? (tcheck "proc(x: pairof int * int) unpair y z = x in zero?(y)") '((pairof int int) -> bool) "pair proc")
 (check-equal? (tcheck "proc(x: int) newpair(x,1)") '(int -> (pairof int int)) "proc return pair")
         
))
(run-tests tests-for-pair 'verbose)
