  ...
  (define type-of
    (lambda (exp tenv)
      (cases expression exp
        ...
        ;new added for pair-exp
        (pair-exp (exp1 exp2)
                  (let ((ty1 (type-of exp1 tenv))
                        (ty2 (type-of exp2 tenv)))
                    (pairof-type ty1 ty2)))
        ;new added for unpair-exp
        (unpair-exp (var1 var2 p-exp body)
                    (let* ((pair-tp (extract-from-pairof-type (type-of p-exp tenv)))
                           (ty1 (car pair-tp))
                           (ty2 (cadr pair-tp)))
                      (type-of body
...
;extract type from pairof-type
(define extract-from-pairof-type
  (lambda (ty)
    (cases type ty
      (int-type () '())
      (bool-type () '())
