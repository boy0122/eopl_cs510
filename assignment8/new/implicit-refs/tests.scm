...
(define implicit-ref-tests
  (test-suite
   "tests for IMPLICIT-REFS language"
   
   (check-equal? (run "begin 1; 2; 3 end") 3 "begin-test-1")

   ;; extremely primitive testing for mutable variables

   (check-equal? (run "let x = 17 in begin set x = 27; x end")
                  27 "assignment-test-1")
...
;;                  720)
      ...
      (check-equal? (run
        "letrec even(odd) = proc(x) if zero?(x) then 1 else (odd -(x,1))
         ...
         1
        "HO-nested-letrecs")
      (check-equal? (run "letrec time(x, y)
             = if zero?(x) then 0 else -((time -(x,1)  y), -(0, y))
      in (time 3 2)") 6 "multiple arguments")
))

(define proc-tests     
  (test-suite
   "tests for PROC language"
    (check-equal? (run "(proc(x,y) *(x,y)  5 6)") 
                  30 "apply-proc-in-rator-pos")
...
(define let-tests
  (test-suite
   ...
   ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
   ;;;;;;;new test case for add, multi and not expression;;;;;;;;;;;;;;
   ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
   (check-equal? (run "+(1,x)") 11 "test add")
   (check-equal? (run "*(2,x)") 20 "test multi")
   (check-equal? (run "not(zero?(0))") #f "test not")
