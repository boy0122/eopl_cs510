...
(define let-tests
  (test-suite
   ...
   ;new added tests
   ;test sign-exp
   (check-equal? (run "sign(zero?(0), 3)") 3 "positive sign")
   (check-equal? (run "sign(zero?(1), 3)") -3 "negative sign")
   ;test plus-exp
   (check-equal? (run "+(2 , v)") 7 "adding")
   ;test minus-exp
   (check-equal? (run "minus(v)") -5 "negative integer")
   ;test less?-exp
   (check-equal? (run "less?(i , v)") #t "comparision")
