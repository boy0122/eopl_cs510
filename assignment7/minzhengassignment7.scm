...
;(define more-for-stm
 ;  (lambda (stm) (> (num-fors stm) (num-whiles stm))))
;modified
(define more-for-stm
  (lambda (stm) (apply > (helper-count stm))))
...
;helper-count: statement?->list
;usgae: (helper-count s) = (f w) with f is the number of for loops and w is the number of while loops
(define helper-count 
  (lambda (stm)
    (cases statement stm
     (while-stmt (wh-expr wh-body) (let ((re-li (helper-count wh-body))) 
                                     (list (car re-li)
                                           (+ 1 (cadr re-li)))))
                                     
     (for-stmt (for-var for-lower for-upper for-body) (let ((re-li (helper-count for-body))) 
                                                        (cons (+ 1 (car re-li))
                                                              (cdr re-li))))
                                                      
     (begin-stmt (begin-first begin-rest) (let ((re-li (cons (helper-count begin-first) (map helper-count begin-rest))))
                                            (list (apply + (map car re-li))
                                                  (apply + (map cadr re-li)))))
      (if-stmt (if-cond if-then if-else) (let ((re-li (list (helper-count if-then) (helper-count if-else))))
                                           (list (apply + (map car re-li)) 
                                                 (apply + (map cadr re-li)))))
      (assign-stmt (var expr) (list 0 0))
      (try-stmt (t-try t-catch)  (let ((re-li (list (helper-count t-try) (helper-count t-catch))))
                                           (list (apply + (map car re-li))
                                                 (apply + (map cadr re-li)))))
      )))
